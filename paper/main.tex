...
might be promoted to a dynamical field in some contexts \cite{wider2020, wang2016, ooguri2012instability, li_dynamical_2010}. It is unlikely that the axion field would have its own independent dynamics and quasiparticles; instead, $\theta_{\text{CS}}$ 
...
Similarly, $\partial_\lambda \theta_{\text{CS}}$ can be expressed as a 3D $k$-space integral of a gauge-invariant four-curvature,
\begin{equation}
...
Unlike the Chern-Simons 3-form in Eq.~\ref{eq:CS3form}, this expression is manifestly gauge invariant and does not require the construction of a globally smooth Bloch-state gauge.

The goal of this project is to develop and code practical methods for computing the linear response of $\theta_{\text{CS}}$ to axion-active phonons that modulate the axion angle away from its symmetry-enforced quantized values. The main focus will be on performing this in the first-principles context using density-functional theory. The $\lambda$-perturbation will be included using methods from \cite{lihm2021}.

\section{Axion-active phonons}

% rough sketch of approach
To modulate the axion angle, we must seek phonon modes that break the underlying $\theta$-quantizing symmetries. Using the SMODES software, we look at the irreducible representations at the $Z$-point, and identify four different symmetry modes of the $T2-$ little group. These break the underlying $\Theta$ symmetry that protects $\theta$ and couples to the axion angle.

We use QuantumESPRESSO to compute the ab initio electronic structure in the unperturbed system, and choose four different displacement amplitudes for each of the four symmetry modes. We then displace the atoms by hand, and compute the ab initio band structure for each displacement amplitude and symmetry mode. After each, we use Wannier90 to compute the Wannierized tight-binding model in PythTB. From these models, we compute the four curvature and the first-order response of the axion angle.
